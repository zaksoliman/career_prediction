...
class Model:
    ...
    def __init__(self, train_inputs, train_targets, test_inputs, test_targets, n_titles, vocab_size, class_mapping=None,
                 ...
                 use_embedding=False, max_grad_norm=5, embedding_dim=100, rnn_cell_type='LSTM', max_timesteps=33,
    ...
    def _predict(self):
        ...
        self.input_data_ids = tf.placeholder(tf.float32, [None, self.max_timesteps], name="titles_id_seq")
        self.target_inputs = tf.placeholder(tf.int32, [None, self.max_timesteps], name="targets")
        # matrix that will hold the length of out sequences
        self.seq_lengths = tf.placeholder(tf.int32, [None], name="seq_lengths")
        # Do embedding
        with tf.device("/cpu:0"):
            one_hot_lookup = tf.Variable(tf.eye(self.n_titles, dtype=tf.int32), trainable=False,
                                         name="target_one_hot_lookup")
            # tile_emb_input has shape batch_size x times steps x emb_dim
            self.targets = tf.nn.embedding_lookup(one_hot_lookup, self.target_inputs, name="one_hot_targets")
    ...
    def _loss(self):
        with tf.name_scope("xent"):
            ...
            mask = tf.sequence_mask(self.seq_lengths, maxlen=self.max_timesteps,
                                    dtype=tf.int32)  # tf.sign(tf.reduce_max(tf.abs(self.targets), 2))
    ...
    def train(self):
        ...
        train_batcher = BOW_Batcher(batch_size=self.batch_size, step_num=self.max_timesteps,
            ...
            for e in range(self.n_epochs):
                ...
                for tb in range(test_batcher.max_batch_num):
                    ...
                    test_acc, test_top_2, test_top_3, test_top_4, test_top_5, test_summ, pred = sess.run([
                        ...
                        {
                            self.input_data_ids: test_title_seq,
