...
class Model:
    ...
    def __init__(self, train_inputs, train_targets, test_inputs, test_targets, n_titles, n_labels, class_mapping=None,
                 use_dropout=True, num_layers=1, keep_prob=0.5, hidden_dim=512, freeze_emb=False, max_grad_norm=5,
                 embedding_dim=300, rnn_cell_type='LSTM', max_timesteps=33, learning_rate=0.001, batch_size=100,
                 n_epochs=800, log_interval=20, store_model=True, restore=True, emb_path="",
        ...
        self.hparams = f"{name}_fasttext_{rnn_cell_type}_{num_layers}_layers_cell_lr_{learning_rate}_freeze_emb={freeze_emb}_emb_dim={embedding_dim}_" \
    ...
    def train(self):
        ...
        train_batcher = Batcher(batch_size=self.batch_size, step_num=self.max_timesteps, input_data=self.train_inputs,
                                    target_data=self.train_targets, n_classes=self.n_labels)
    ...
    def test(self):
        ...
        base_path = "/data/rali7/Tmp/solimanz/data/multilabel_model_predictions"
